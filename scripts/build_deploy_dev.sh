...
REPO_NAME="predix-edge-sample-scaler-cpp"
DOCKER_STACK_NAME="predix-edge-sample-scaler-cpp"
...
if [[ "$SKIP_PREDIX_SERVICES" == "false" ]]; then
  ...
  QUICKSTART_ARGS="$QUICKSTART_ARGS $SCRIPT -uaa -ts -psts -psmodel $PS_MODEL -repo-name $REPO_NAME -app-name $DOCKER_STACK_NAME"
fi
...
processDockerCompose docker-compose-dev.yml
docker stack deploy -c docker-compose-dev.yml edge-app
